...
if(validate($id, $auth_token)){
  ...
  $db="u157368432_acn";  
  $sentencia = "SELECT ev.EVENT_ID, op.URL, opb.OPPONENT_NAME  'opb.OPPONENT_NAME', NAME, SURNAME, opb.URL 'opb.URL', ope2.ODDS, bet.OPP_USER_ID, cat.CATEGORY_ID, bet.BET_DTTM, bet.AMOUNT ,cat.DESCRIPTION as 'DESC', scat.DESCRIPTION, scat.SUB_CATEGORY_ID, ev.EVENT, ev.OFF_DTTM, ev.EVENT_TYPE FROM $db.CM_EVENT ev  
				  ...
				  WHERE ev.EVENT_STATUS_FLG IN ('C','O') AND ev.OFF_DTTM <= '".NOW()."'
				  and ope.OPPONENT_ID != 0
				  ORDER BY ev.OFF_DTTM, cat.CATEGORY_ID, scat.SUB_CATEGORY_ID, ev.EVENT_ID, ope.LOCAL"; 
